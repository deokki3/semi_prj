...
public class DisLikeController extends HttpServlet {
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(!r) {
			...
			//싫어요 눌렀을때 상태를 -1 & 좋아요 -1
			
			dao.dupdate(members_id_fk, movie_ls_num_fk);
...
//		if(!(members_id_fk.equals(vo.getMembers_id_fk()))) { 
//			int n=dao.insert(vo);
//			dao.dupdate(movie_ls_num_fk);
//			if(n>0) {
//				req.setAttribute("status", -1);  
//				req.getRequestDispatcher("/movieDetail").forward(req, resp); 
//			}
...
//				req.getRequestDispatcher("/movieDetail").forward(req, resp); } 
//			}
//		else if(vo.getLike_or_dislike()==-1){
//			dao.cancel(movie_ls_num_fk);
//			req.setAttribute("status", 0); 
//			req.getRequestDispatcher("/movieDetail").forward(req, resp); 
//		}
//		else {
